...
void sort(struct Appointment appoint[], int maximum) {
    int count, flag;
    struct Appointment temp;

    // Storing all the date and time values in minutes for comparison
    for (count = 0; count < maximum; count++) {
        appoint[count].time.min = (appoint[count].date.year * 12 * 30 * 24 * 60) + (appoint[count].date.month * 30 * 24 * 60) + (appoint[count].date.day * 24 * 60) + (appoint[count].time.hour * 60) + appoint[count].time.min;
    }

    // Using bubble sort to compare minutes and then swap the structs
    for (count = maximum - 1; count > 0; count--)
    {
        for (flag = 0; flag < count; flag++)
        {
            if (appoint[flag].time.min > appoint[flag + 1].time.min)
            {
                temp = appoint[flag];
                appoint[flag] = appoint[flag + 1];
                appoint[flag + 1] = temp;
            }
        }
    }
    ...
    for (count = 0; count < maximum; count++) {
        appoint[count].time.min = appoint[count].time.min - (appoint[count].date.year * 12 * 30 * 24 * 60) - (appoint[count].date.month * 30 * 24 * 60) - (appoint[count].date.day * 24 * 60) - (appoint[count].time.hour * 60);
    }
...
void displayAllPatients(const struct Patient patient[], int max, int fmt) {
    ...
    for (k = 0; k < max; k++) {
        if (patient[k].patientNumber != 0) {
            displayPatientData(&patient[k], fmt);
            record = 1;
        }
    }
...
void viewAllAppointments(struct ClinicData* data)
{
    ...
    for (i = 0; i < data->maxAppointments; i++) {
        for (k = 0; k < data->maxPatient; k++) {
            if (data->appointments[i].patientNumber && data->patients[k].patientNumber) {
                if (data->appointments[i].patientNumber == data->patients[k].patientNumber) {
                    displayScheduleData(&data->patients[k], &data->appointments[i], 1);
                }
            }
        }
    }
...
void addAppointment(struct Appointment *appointment, int maxAppointments, struct Patient *pt, int maxPatients) {
    ...
    if (index >= 0) {
        ...
        index = nextSlotAvailable(appointment, maxAppointments);
...
void removeAppointment(struct Appointment *appointment, int maxAppointments, struct Patient *pt, int maxPatients) {
    ...
    if (index >= 0) {
        ...
        aptIndex = validAppointment(patientNumber, date, appointment, maxAppointments);
...
void searchPatientByPatientNumber(const struct Patient patient[], int max) {
    ...
    // Checking if patient number exists
    if (findPatientIndexByPatientNum(patient_Num, patient, max) != -1) {
